class VegetableStore {
    ...
    buyingVegetables(selectedProducts) {
        ...
        selectedProducts.forEach(data => {
            ...
            let currentProduct = this.availableProducts.find(el => el.type === type);
            if (!currentProduct) {
    ...
    rottingVegetable(type, quantity) {
        let currentType = this.availableProducts.find(el => el.type === type)
        if (!currentType) {
    ...
    revision() {
        ...
        .forEach(el => buff+=`${el.type}-${el.quantity}-$${el.price}\n`)
